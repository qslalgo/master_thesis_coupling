...
from power_extraction import build_qprime_pin_for_solver_by_assembly, build_qprime_pin_for_solver_by_assembly_new
...
def log_iteration(log_file, it, th_results):
    ...
    for assy_id in sorted(th_results.keys()):
        ...
        log_file.write(
            ...
            # f"f_in={frac_in:.2f}, "
            # f"f_out={frac_out:.2f}\n"
            f"mdot_in={mdot_in:.6e}, "
            f"mdot_out={mdot_out:.6e}\n"
        )
...
def full_feedback_loop(base_dir, mesh_path, case_name, config, n_iter=6):
    ...
    T_init = getattr(config, "T_inlet_K", 293.15)
    ...
    for it in range(max_iter):
        ...
        for assy_id, data in inner_coolant_layers_by_assembly.items():
            ...
            # lower
            data["lower"].temperature = float(T_inner[assy_id][0])

            inner_coolant_boundary_materials[assy_id]["lower"].set_density(
                "g/cm3",
                float(rho_inner[assy_id][0])
            )
        ...
        for assy_id, assy_type in assembly_types.items():
            ...
            qprime_data_assy = qprime_by_assembly[assy_id]     # Power profile (W/m)
        ...
        converged = (
            ( max_dT_fuel_rel   < tol_fuel_rel) and
            ( max_dT_inner_rel  < tol_innerT_rel))
            #and
...
if __name__ == "__main__":
    ...
    mesh_path = "/home/faticen/master_thesis/coupling/meshes"

    case_name = "high_power"
    config = cases[case_name]


    log_path = os.path.join("/home/faticen/master_thesis/coupling/", f"case_{case_name}", "iteration_log.txt")
