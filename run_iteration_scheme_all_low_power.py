...
def log_iteration(log_file, it, th_results):
    ...
    for assy_id in sorted(th_results.keys()):
        T_inner = th_results[assy_id]["T_inner_z_K"]
        rho_in = th_results[assy_id]["rho_inner_z"]
        # v_out = th_results[assy_id]["v_outer"]
...
def run_th_case(config, mesh_path, qprime_data, case_name, outdir):
    ...
    solver.run(case_name=case_name, out_dir=outdir)
